...
triggers:
  ...
  - diff
  - compare pages
allowed-tools: Bash(agent-browser:*), Bash(ab:*)
---

# CRITICAL SAFETY RULES

**NEVER run `agent-browser close`.** `ab` blocks this command. It kills Chrome ungracefully, corrupts the profile, and puts the daemon in an unrecoverable state.

**NEVER run `pkill chrome` or `pkill -f "chromium|chrome"`.** The user's personal Chrome uses the same binary. You will kill their browser, tabs, and unsaved work. Only `ab heal` can safely kill the managed Chrome instance.

**NEVER use `eval` for interaction or inspection.** Don't run `eval "document.querySelector(...).click()"` or `eval "document.body.innerText"`. Use the proper commands: `ab snapshot -i` for element discovery, `ab click @e1` / `ab find text "X" click` for interaction, `ab get text @e1` for reading content. `eval` is only for JavaScript that has no agent-browser equivalent (e.g., reading `localStorage`, computing derived values).

**NEVER call `agent-browser` directly.** Always use `ab`. The `ab` wrapper ensures every call goes through CDP with the correct flags (`--cdp 9333`, `AGENT_BROWSER_AUTO_CONNECT=` unset). Calling `agent-browser` directly may spawn a rogue daemon or hang.
...
Worktrees run on unique ports to avoid collisions with the main dev server. Use portless URLs for browser access.

| Context | URL | Notes |
|---------|-----|-------|
| Main dev | `terra.localhost` | Port 5173 via portless |
| Worktree dev | `<name>.terra.localhost` | Unique ports via worktree-ports.js |
| Staging | `slack-feedback-staging.onrender.com` | |
| Production | `terra.clay.com` | |

**Starting a worktree dev server:** `bun run dev:worktree` (from the worktree root) starts backend + Vite on unique ports with a portless URL alias.
...
# NOTE: `close` is BLOCKED by ab — use `ab heal` to reset browser state
...
Auto-connect discovers Chrome via `DevToolsActivePort`, common debugging ports (9333, 9229), and falls back to a direct WebSocket connection if HTTP-based CDP discovery fails.
...
**Always use `ab record`, never `agent-browser record` directly.** The raw `record start` creates a new browser context that loses auth (localStorage, session tokens). `ab record` works around this by recording the active tab directly.

```bash
ab record start /tmp/output.webm    # Starts recording the active tab
# ... interact with the page ...
ab record stop                       # Stops and saves the recording
```
...
```json
{ "default": "deny", "allow": ["open", "snapshot", "click", "scroll", "wait", "get"] }
```
